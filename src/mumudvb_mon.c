...
{
	static char short_options[] = "c:a:hlhq";//q;//c:sdthvql";//;
	static struct option long_options[] = {
			{"freq", required_argument, NULL, 'f'},
			{"pol", required_argument, NULL, 'p'},
			{"delivery_system", required_argument, NULL, 'd'},
			{"srate", required_argument, NULL, 's'},
			{"lnb_type", required_argument, NULL, 't'},
			{"lnb_lof_standard", required_argument, NULL, 'z'},
			{"lnb_slof", required_argument, NULL, 'x'},
			{"lnb_lof_low", required_argument, NULL, 'v'},
			{"lnb_lof_high", required_argument, NULL, 'b'},
			{"modulation", required_argument, NULL, 'm'},
			{"coderate", required_argument, NULL, 'r'},
			{"sat_number", required_argument, NULL, 'n'},
	...
	while (1)
	{
		...
		case 'p':
		    if (long_options[option_index].flag != 0)
	                break;
		    int len = strlen (long_options[option_index].name) + strlen (optarg) + 2;
		    char *conf_line = malloc (len);
		    //char conf_line[255];
		    sprintf( conf_line, "%s=%s",long_options[option_index].name,optarg);
		...
		card_buffer_t *card_buffer)
{
	...
	log_message( log_module,  MSG_INFO,"========== DVBZAP version %s is stopping with ExitCode %d ==========",VERSION,ExitCode);
